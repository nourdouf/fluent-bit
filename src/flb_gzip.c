...
static int gzip_stream_fail(struct flb_gzip_stream *stream)
{
    if (stream) {
...
static int gzip_stream_reserve(struct flb_gzip_stream *stream, size_t extra)
{
    size_t needed;
    size_t capacity;
    void *body;
...
static int gzip_stream_pump(struct flb_gzip_stream *stream, int flush)
{
...
struct flb_gzip_stream *flb_gzip_stream_create(void)
{
    ...
    if (status != MZ_OK || gzip_stream_reserve(stream, FLB_GZIP_HEADER_SIZE) != 0) {
        flb_gzip_stream_destroy(stream);
        return NULL;
    }
    gzip_header(stream->body);
    stream->size = FLB_GZIP_HEADER_SIZE;
